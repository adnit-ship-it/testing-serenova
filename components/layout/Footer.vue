        ...
        <NuxtLink to="/">
          <div class="pb-1" :style="{ height: logoHeight }">
            <img
              ...
              :alt="common?.accessibility?.brandLogo"
              class="h-full w-full"
            />
          </div>
        </NuxtLink>
...
const getPageLayout = computed(() => {
  ...
  if (!pageName) return common.value?.layout;
  
  const pageData = siteTextStore.siteText?.[pageName];
  return pageData?.layout || common.value?.layout;
});
